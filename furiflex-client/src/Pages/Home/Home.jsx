    ...
    <div className="flex">
      ...
      <div className="w-1/4 h-screen bg-gray-100 p-4 sticky top-0">
        ...
        {categoryArray.map((category, index) => (
            <li className=" whitespace-nowrap py-2 hover:text-primary cursor-pointer border-b-2" onClick={() => handleCategory(category)} key={index}>
              {category}
            </li>
          ))}
        </ul>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          {products?.slice(0,6).map((item, idx) => (
            <div key={idx} className="border p-4 rounded-lg shadow">
              ...
              <button onClick={handleAddCart} className="mt-4 bg-black text-white py-2 px-4 w-full rounded">
                Add to cart
              </button>
            </div>
          ))}
